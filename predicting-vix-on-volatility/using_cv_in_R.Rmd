...
```{r data}
# First, get the S&P500 prices from 1/2/2008 - 1/1/2024
spy <- tq_get("SPY",
              get = "stock.prices",
              from = "2008-01-02",
              to = "2024-01-01")
...
vix_volatility |>
  ...
  labs(title = "Realized Volatility and VIX Over 2023",
       x = "Month", y = "Volatility Percentage", color = "") +
  theme_bw()
...
vix_volatility |>
  ...
  labs(title = "VIX vs Realized Volatility of 2023",
...
vix_volatility |>
  ...
  geom_point(aes(y = predict(lm.mdl, newdata = na.omit(vix_volatility)), color = "Linear"), alpha = 0.5) +
  geom_point(aes(y = predict(kernel.mdl, newdata = na.omit(vix_volatility)), color = "Kernel"), 
             alpha = 0.5) +
  geom_point(aes(y = predict(gam.mdl, newdata = na.omit(vix_volatility)), color = "GAM"), alpha = 0.5) +
